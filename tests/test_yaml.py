...
"""
    ...
    def _render_func(fobj: IO[str], *_: Any) -> IO[str]:
        """
        Dummy function to render a path.
        """
    ...
    with open(path, "w+", encoding="utf-8") as file_obj:
        ...
        yaml_dict = YAML.load(file_obj, render=render)
        assert yaml_dict == {
            "cm": {
                "sut1": {
                    "type": "qemu",
...
"""
    ...
    with open(tmp_path / "file1.yaml", "w+", encoding="utf-8") as file_obj1, \
        ...
        yaml_dict = YAML.load(file_obj1, basepath=tmp_path)
        assert yaml_dict == {
            "cm": {
                "sut1": {
                    "type": "baremetal",
