...
def test_aspm_info(params: CommonTestParamsTypedDict):
    ...
    for option in good:
        _PropsCommonCmdl.run_pepc(f"aspm info {option}", pman)

    # Error: bad device address.
    _PropsCommonCmdl.run_pepc("aspm info --device bad:dev:addr.0", pman, exp_exc=Error)
    _PropsCommonCmdl.run_pepc("aspm info --policy --policies --device bad:dev:addr.0", pman,
                             exp_exc=Error)

    device = _get_l1_device(pman)
    _PropsCommonCmdl.run_pepc(f"aspm info --device {device}", pman)
    _PropsCommonCmdl.run_pepc(f"aspm info --policy --policies --device {device}", pman)
...
def test_aspm_config(params):
    ...
    _PropsCommonCmdl.run_pepc("aspm config", pman, exp_exc=Error)

    # Error: bad policy name, no superuser or passwordless sudo needed.
    _PropsCommonCmdl.run_pepc("aspm config --policy badpolicyname", pman, exp_exc=Error)

    # Display the current ASPM policy without changing it.
    _PropsCommonCmdl.run_pepc("aspm config --policy", pman)
    ...
    for option in good:
        try:
            _PropsCommonCmdl.run_pepc(f"aspm config {option}", pman, re_raise=True)
...
def test_aspm_info_l1(params: CommonTestParamsTypedDict):
    ...
    _PropsCommonCmdl.run_pepc("aspm info --l1-aspm", pman, exp_exc=Error)

    device = _get_l1_device(pman)
    _PropsCommonCmdl.run_pepc(f"aspm info --l1-aspm --device {device}", pman)
...
def test_aspm_config_l1(params: CommonTestParamsTypedDict):
    ...
    _PropsCommonCmdl.run_pepc("aspm config --l1-aspm on", pman, exp_exc=Error)

    device = _get_l1_device(pman)

    # Error paths that need a device address but not superuser or passwordless sudo.
    _PropsCommonCmdl.run_pepc(f"aspm config --l1-aspm badval --device {device}", pman,
                             exp_exc=Error)
    _PropsCommonCmdl.run_pepc("aspm config --l1-aspm on --device bad:dev:addr.0", pman,
                             exp_exc=Error)
    _PropsCommonCmdl.run_pepc(f"aspm config --policy performance --device {device}", pman,
                             exp_exc=Error)
    _PropsCommonCmdl.run_pepc(f"aspm config --policy performance --l1-aspm badval --device "
                             f"{device}", pman, exp_exc=Error)

    # Display the current L1 ASPM state and ASPM policy without changing them.
    _PropsCommonCmdl.run_pepc(f"aspm config --l1-aspm --device {device}", pman)
    _PropsCommonCmdl.run_pepc(f"aspm config --policy --l1-aspm --device {device}", pman)
    ...
    for val in ("on", "off", "enable", "disable", "true", "false"):
        _PropsCommonCmdl.run_pepc(f"aspm config --l1-aspm {val} --device {device}", pman)

    _PropsCommonCmdl.run_pepc(f"aspm config --policy performance --l1-aspm on --device {device}",
                             pman)
