...
This is one of the most perplexing aspects of MSR scope and the primary reason for this article.
I spent significant amount of time understanding unexpected platform behavior while developing `pepc`
...
helps in understanding how MSR_BLAH operates.

**Disclaimer**: This mental model is my own creation and may not accurately represent the actual
hardware implementation.

Imagine a variable 'var0' inside package 0 and variable 'var1' inside package 1. MSR_BLAH is an
interface for the CPU to write to these variables:
...
Imagine there are per-core "cache" variables for 'var0' and 'var1'. In our abstract platform, this
would mean:
...
On Sierra Forest Xeon and Grand Ridge SoC:
...
Since the last write to the MSR set 'pkg_cstate_limit=2' (PC6) on CPU 0, we know this is the value
...
Now let's involve CPU1, and set the package C-state limit to PC0 by writing
pkg_cstate_limit=0 to MSR_PKG_CST_CONFIG_CONTROL on CPU 1.

```bash
# What limit does CPU 1 report?
$ rdmsr --processor 1 0xE2
10000400
