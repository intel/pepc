...
class FeaturedMSR(ClassHelpers.SimpleCloseContext):
    ...
    def _init_msr_sname(self):
        """
        Initialize MSR scope.

        Often MSR scope matches scope of features controlled by the MSR, but this is not always the
        case. For example, MSR_PKG_CST_CONFIG_CONTROL includes both a package-scope "Package C-state
        limit" feature and a core-scope "C1 demotion" feature. By default, we use the "smallest"
        feature scope as the MSR scope.
        """

        scopes = {sname : idx for idx, sname in enumerate(MSR.SCOPES)}
        self.msr_sname = MSR.SCOPES[-1]

        for finfo in self._features.values():
            sname = finfo["sname"]
            if scopes[sname] < scopes[self.msr_sname]:
                self.msr_sname = sname

        self._msr.set_msr_scope(self.regaddr, self.msr_sname)

        _LOG.debug("%s (%#x) scope is set to '%s'", self.regname, self.regaddr, self.msr_sname)
    ...
    def _init_features_dict(self):
        ...
        if not self.msr_sname:
            self._init_msr_sname()
        elif self.msr_sname not in MSR.SCOPES:
            raise Error(f"BUG: bad scope name '{self.msr_sname}'")
    ...
    def __init__(self, pman=None, cpuinfo=None, msr=None):
        ...
        # Scope that has to be used for MSR I/O.
        self.msr_sname = None
