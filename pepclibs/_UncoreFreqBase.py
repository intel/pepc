...
class UncoreFreqBase(ClassHelpers.SimpleCloseContext):
    ...
    def _validate_elc_threshold(self,
        ...
        if thrtype == "low":
            ...
            if threshold > high_threshold:
                raise ErrorBadOrder(f"Cannot set the ELC low threshold to value '{threshold}': "
                                    f"it is higher than the currently configured ELC high "
                                    f"threshold value '{high_threshold}'")
        elif thrtype == "high":
            _, _, low_threshold = next(self._get_elc_threshold_dies("low", {package: [die]}))
            if threshold < low_threshold:
                raise ErrorBadOrder(f"Cannot set the ELC high threshold to value '{threshold}': "
                                    f"it is lower than the currently configured ELC low "
                                    f"threshold value '{low_threshold}'")
        else:
            raise Error(f"BUG: bad ELC threshold type '{thrtype}'")

    def _set_elc_threshold_dies(self, threshold: int, thrtype: ELCThresholdType, dies: RelNumsType):
        """
    ...
    def _set_elc_threshold_cpus(self, threshold: int, thrtype: ELCThresholdType, cpus: AbsNumsType):
        """
