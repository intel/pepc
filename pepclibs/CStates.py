    ...
    def _read_fpaths_and_values(self, cpus):
        ...
        if not files:
            ...
            with contextlib.suppress(Error):
                with CStates(pman=self._pman, cpuinfo=self._cpuinfo, cpuidle=self) as csobj:
...
class CStates(_PCStatesBase.PCStatesBase):
    ...
    def _get_cpuidle(self):
        ...
        if not self._cpuidle:
            self._cpuidle = CPUIdle(self._pman, cpuinfo=self._cpuinfo)
    ...
    def __init__(self, pman=None, cpuinfo=None, cpuidle=None, msr=None, enable_cache=True):
        ...
        The class constructor. The arguments are as follows.
          ...
          * cpuinfo - CPU information object generated by 'CPUInfo.CPUInfo()'.
          * cpuidle - a 'CStates.CPUIdle()' object which should be used for reading and setting
