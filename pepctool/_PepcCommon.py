...
def get_cpus(args, cpuinfo, default_cpus="all", offlined_ok=False):
    """
    Get list of CPUs based on requested CPU, core, package numbers. The arguments are as follows.
      ...
      * default_cpus - used only when 'args' does not include any CPU, core, or package numbers,
                       supposedly because the user did not provide them via command-line arguments.
                       Specifies an integer list of CPU numbers to use in such case. Value "all"
                       means "all CPUs".
    ...
    But if this function ends up expanding special value "all" for 'args.cores' or 'args.packages',
    then 'offline_ok' argument is not used (ignored). The reason for this is that Linux kernel does
    not provide topology information for offline CPUs. Only offline CPU numbers are known, but what
    package or core they belong to is unknown, and becomes known only when the CPU is onlined.
    """
